...
public class ScoresToResults implements MapGroupsFunction<Query, Tuple2<Query, RankedResult>, DocumentRanking>{
    ...
    @Override
    public DocumentRanking call(Query key, Iterator<Tuple2<Query, RankedResult>> values)
            throws Exception {
        ...
        while (resultsList.size() <= 10 && resultIterator.hasNext()) {
            ...
            if (currResult.getArticle().getTitle() != null && currResult.getArticle().getTitle().isEmpty() == false){
                if (resultsList.isEmpty()) {
                    ...
                    for (RankedResult item : resultsList) {
                        ...
                        if (TextDistanceCalculator.similarity(currResult.getArticle().getTitle(), item.getArticle().getTitle()) >= 0.5) {
                            toAdd = false;
                            break;
                        }
                    }
